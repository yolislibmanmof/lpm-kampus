<?php
require_once __DIR__ . '/includes/lang.php';   // ← WAJIB: muat t() DULU
$pageTitle = t('home', 'Beranda');
...
// ===== SMART SLIDER PARSER =====
$rawSlider = Site::setting('hero_slider', '');
$sliderImgs = [];
$sliderMeta = [];
...
if (!empty($rawSlider)) {
    ...
    if (is_array($decoded) && isset($decoded[0]['path'])) {
        foreach ($decoded as $item) {
            if (($item['active'] ?? true) && !empty($item['path'])) {
                $sliderImgs[] = $item['path'];
                $sliderMeta[$item['path']] = [
...
.ix-gs-spark { height: 18px; margin-top: 8px; opacity: .6; }
...
/* ===== STATS WALL ===== */
.ix-stats-wall { background: #fff; padding: 100px 0; position: relative; }
.ix-sw-head { text-align: center; max-width: 720px; margin: 0 auto 72px; }
.ix-sw-tag {
...
.ix-sc-ring { width: 80px; height: 80px; margin: 0 auto 20px; position: relative; }
...
.ix-news-grid { display: grid; grid-template-columns: 1.4fr 1fr 1fr; gap: 22px; }
.ix-news-hero {
...
.ix-quote cite { display: block; font-style: normal; }
...
/* Strip 4 kartu */
.ix-strip { position: relative; overflow: hidden; padding: 72px 0; background: linear-gradient(180deg, #061D2E, #092A40); }
...
<section class="ix-hero" id="hero">
    ...
    <div class="container">
        <div class="ix-hero-inner">
            ...
            <span class="ix-eyebrow">
                ...
                <?= t('hero_eyebrow', 'Lembaga Penjaminan Mutu Internal') ?>
            </span>
            <!-- 🌐 LANG: Hero title -->
            <h1>
                <?= t('hero_title_1', 'Menjaga Kualitas,') ?><br>
                <?= t('hero_title_2', 'Meraih') ?> <span class="ix-grad"><?= t('hero_title_highlight', 'Keunggulan Akademik') ?></span>
            </h1>
            <!-- 🌐 LANG: Hero lead -->
            <p class="ix-hero-lead"><?= t('hero_lead') ?></p>
            <!-- 🌐 LANG: Hero CTA -->
            <div class="ix-hero-ctas">
                <a href="/publik/profil.php" class="ix-btn-glow ix-btn-primary"><?= t('hero_cta_about', '🏛️ Tentang Kami') ?></a>
                <a href="/publik/akreditasi.php" class="ix-btn-glow ix-btn-ghost"><?= t('hero_cta_accreditation', '🏆 Lihat Akreditasi →') ?></a>
            </div>
            <!-- 🌐 LANG: Glass stats -->
            <div class="ix-glass">
                <div class="ix-gs">
                    <span class="ix-gs-num" data-count="<?= $stats['total'] ?? 0 ?>">0</span>
                    <span class="ix-gs-label"><?= t('program_studies', 'Program Studi') ?></span>
                ...
                <div class="ix-gs">
                    ...
                    <span class="ix-gs-label"><?= t('hero_stat_excellent', 'Unggul / A') ?></span>
                    <svg class="ix-gs-spark" viewBox="0 0 100 20" preserveAspectRatio="none">
                ...
                <div class="ix-gs">
                    ...
                    <span class="ix-gs-label"><?= t('hero_stat_very_good', 'Baik Sekali') ?></span>
                    <svg class="ix-gs-spark" viewBox="0 0 100 20" preserveAspectRatio="none">
                ...
                <div class="ix-gs">
                    ...
                    <span class="ix-gs-label"><?= t('hero_stat_stdikti', 'Standar SN-Dikti') ?></span>
                    <svg class="ix-gs-spark" viewBox="0 0 100 20" preserveAspectRatio="none">
    ...
    <div class="ix-scroll-hint">
        <!-- 🌐 LANG: Scroll hint -->
        <span><?= t('scroll_hint', 'Scroll') ?></span>
    ...
    <div class="container">
        <div class="ix-strip-grid">
            ...
            <div class="ix-strip-card">
                ...
                <div><strong><?= t('strip_1_title', 'Akreditasi') ?></strong><small><?= t('strip_1_desc', 'Unggul / A') ?></small></div>
            </div>
            <div class="ix-strip-card">
                <div class="ix-strip-ic">📊</div>
                <div><strong><?= t('strip_2_title', 'Dasbor Real-time') ?></strong><small><?= t('strip_2_desc', 'PPEPP Dashboard') ?></small></div>
            </div>
            <div class="ix-strip-card">
                <div class="ix-strip-ic">🎯</div>
                <div><strong><?= t('strip_3_title', 'AMI Aktif') ?></strong><small><?= t('strip_3_desc', '100% Prodi') ?></small></div>
            </div>
            <div class="ix-strip-card">
                <div class="ix-strip-ic">🔍</div>
                <div><strong><?= t('strip_4_title', 'Auditor Tersertifikasi') ?></strong><small><?= t('strip_4_desc', 'Tim Profesional') ?></small></div>
            </div>
        </div>
    </div>
</section>
...
<div class="ix-trust">
    <!-- 🌐 LANG: Partners label -->
    <div class="ix-trust-label"><?= t('partners_label', 'Terafiliasi & Diakui Oleh') ?></div>
    <div class="ix-trust-track">
        <?php
        $trusts = ['BAN-PT', 'LAM INFOKOM', 'LAM SAINTEK', 'SN-DIKTI 2024', 'KEMENDIKBUDRISTEK', 'PDDIKTI', 'SISTER', 'ISO 21001'];
        for ($i = 0; $i < 2; $i++):
            foreach ($trusts as $t_item): ?>
            <div class="ix-trust-item"><span class="ix-dot"></span> <?= $t_item ?></div>
...
<section class="ix-stats-wall">
    <div class="container">
        <div class="ix-sw-head ix-reveal">
            <!-- 🌐 LANG: Stats wall head -->
            <span class="ix-sw-tag"><?= t('stats_tag', 'Impact dalam Angka') ?></span>
            <h2><?= t('stats_title_1', 'Komitmen Kami Terukur,') ?><br><?= t('stats_title_2', 'Hasil Kami Nyata') ?></h2>
            <p><?= t('stats_desc', 'Data real-time yang menunjukkan perjalanan penjaminan mutu kami.') ?></p>
        </div>
        <div class="ix-sw-grid ix-reveal">
            ...
            <div class="ix-sw-item">
                ...
                <div class="ix-sw-label"><?= t('program_studies', 'Program Studi') ?></div>
                <span class="ix-sw-trend"><?= t('stats_trend_plus', '▲ +2 Tahun ini') ?></span>
            </div>
            <div class="ix-sw-item">
                ...
                <div class="ix-sw-label"><?= t('stats_accredited', 'Akreditasi Unggul') ?></div>
                <span class="ix-sw-trend"><?= t('stats_trend_target', '▲ Target tercapai') ?></span>
            </div>
            <div class="ix-sw-item">
                <div class="ix-sw-ico">🎯</div>
                <div class="ix-sw-num">100<span class="ix-suffix">%</span></div>
                <div class="ix-sw-label"><?= t('stats_ami', 'Prodi Ter-AMI') ?></div>
                <span class="ix-sw-trend"><?= t('stats_trend_ontrack', '● On track') ?></span>
            </div>
            <div class="ix-sw-item">
                <div class="ix-sw-ico">👥</div>
                <div class="ix-sw-num" data-count="24">0</div>
                <div class="ix-sw-label"><?= t('stats_auditors', 'Auditor Aktif') ?></div>
                <span class="ix-sw-trend"><?= t('stats_trend_certified', '▲ Tersertifikasi') ?></span>
            </div>
        </div>
    </div>
</section>
...
<section class="ix-bento-sec">
    <div class="container">
        <div class="ix-sw-head ix-reveal">
            <!-- 🌐 LANG: Tupoksi head -->
            <span class="ix-sw-tag"><?= t('tupoksi_tag', 'Tupoksi Kami') ?></span>
            <h2><?= t('tupoksi_title_1', 'Enam Pilar Penjaminan Mutu') ?><br>yang <span style="color:var(--accent);"><?= t('tupoksi_title_2', 'Saling Terintegrasi') ?></span></h2>
            <p><?= t('tupoksi_desc', 'Struktur kerja LPM dirancang untuk memastikan mutu akademik terjaga di setiap lini.') ?></p>
        </div>
        <div class="ix-bento">
            <!-- 🌐 LANG: 6 tupoksi pillars -->
            <div class="ix-bn ix-bn-hero ix-reveal">
                <span class="ix-bn-num">01</span>
                <div class="ix-bn-hero-eyebrow"><?= t('tupoksi_main', 'Pilar Utama') ?></div>
                <div class="ix-bn-ic">📋</div>
                <h3><?= t('tupoksi_1_title', 'Menetapkan Standar Mutu') ?></h3>
                <p><?= t('tupoksi_1_desc') ?></p>
            </div>
            <div class="ix-bn ix-bn-sm ix-reveal">
                <span class="ix-bn-num">02</span>
                <div class="ix-bn-ic">🔍</div>
                <h3><?= t('tupoksi_2_title', 'Mengawasi Pelaksanaan') ?></h3>
                <p><?= t('tupoksi_2_desc') ?></p>
            </div>
            <div class="ix-bn ix-bn-sm ix-reveal">
                <span class="ix-bn-num">03</span>
                <div class="ix-bn-ic">✅</div>
                <h3><?= t('tupoksi_3_title', 'Audit Mutu Internal') ?></h3>
                <p><?= t('tupoksi_3_desc') ?></p>
            </div>
            <div class="ix-bn ix-bn-xs ix-reveal">
                <span class="ix-bn-num">04</span>
                <div class="ix-bn-ic">🏆</div>
                <h3><?= t('tupoksi_4_title', 'Mengelola Akreditasi') ?></h3>
                <p><?= t('tupoksi_4_desc') ?></p>
            </div>
            <div class="ix-bn ix-bn-xs ix-reveal">
                <span class="ix-bn-num">05</span>
                <div class="ix-bn-ic">📊</div>
                <h3><?= t('tupoksi_5_title', 'Monev Real-time') ?></h3>
                <p><?= t('tupoksi_5_desc') ?></p>
            </div>
            <div class="ix-bn ix-bn-xs ix-reveal">
                <span class="ix-bn-num">06</span>
                <div class="ix-bn-ic">🔄</div>
                <h3><?= t('tupoksi_6_title', 'Peningkatan') ?></h3>
                <p><?= t('tupoksi_6_desc') ?></p>
            </div>
        </div>
    </div>
</section>
...
<section class="ix-tl">
    <div class="ix-tl-inner">
        <div class="ix-tl-head ix-reveal">
            <!-- 🌐 LANG: PPEPP head -->
            <span class="ix-sw-tag"><?= t('ppepp_tag', 'Cara Kami Bekerja') ?></span>
            <h2><?= t('ppepp_title', 'Siklus PPEPP Berjalan Tanpa Henti') ?></h2>
            <p><?= t('ppepp_desc', 'Lima langkah penjaminan mutu yang berulang setiap tahun akademik, membentuk spiral peningkatan berkelanjutan.') ?></p>
        </div>
    ...
    <div class="ix-tl-track" id="tlTrack">
        ...
        // 🌐 LANG: PPEPP 5 steps
        $ppepp = [
            ['📋', t('ppepp_1', 'Penetapan'), t('ppepp_1_desc'), t('ppepp_1_year', 'Tahun ke-1')],
            ['⚙️', t('ppepp_2', 'Pelaksanaan'), t('ppepp_2_desc'), t('ppepp_2_year', 'Berjalan')],
            ['🔍', t('ppepp_3', 'Evaluasi'), t('ppepp_3_desc'), t('ppepp_3_year', 'Tahunan')],
            ['🎯', t('ppepp_4', 'Pengendalian'), t('ppepp_4_desc'), t('ppepp_4_year', 'Triwulanan')],
            ['📈', t('ppepp_5', 'Peningkatan'), t('ppepp_5_desc'), t('ppepp_5_year', 'RTM Tahunan')],
        ];
    ...
    <div class="container ix-sc-inner">
        <div class="ix-sc-head ix-reveal">
            <div class="ix-sc-head-text">
                <!-- 🌐 LANG: Showcase head -->
                <span class="ix-sw-tag" style="background:rgba(201,162,39,.18);color:var(--accent-light);border-color:rgba(201,162,39,.3);"><?= t('showcase_tag', '🏆 Showcase Unggulan') ?></span>
                <h2><?= t('showcase_title_1', 'Program Studi') ?> <span class="ix-grad"><?= t('showcase_title_2', 'Unggulan') ?></span> Kami</h2>
                <p><?= t('showcase_desc', 'Prodi yang telah meraih predikat Unggul / A — bukti konkret komitmen mutu berkelanjutan.') ?></p>
            </div>
            <a href="/publik/akreditasi.php" class="ix-btn-glow ix-btn-primary"><?= t('showcase_all', 'Lihat Semua →') ?></a>
        </div>
        <div class="ix-sc-grid">
            <?php if (empty($unggulList)): ?>
                <div style="grid-column:1/-1;text-align:center;padding:60px;color:rgba(255,255,255,.6);">
                    <div style="font-size:64px;margin-bottom:16px;"><?= t('showcase_empty_title', '🏅') ?></div>
                    <p style="font-size:16px;"><?= t('showcase_empty_text', 'Belum ada prodi dengan predikat Unggul. Mari kita raih bersama!') ?></p>
                </div>
            <?php else: foreach ($unggulList as $u): ?>
                <div class="ix-sc-card ix-reveal">
                    ...
                    <div class="ix-sc-ring">
                        ...
                        <div class="num"><?= t('showcase_rank_label', 'A+') ?></div>
                    </div>
...
<section class="ix-news">
    <div class="container">
        <div class="ix-sw-head ix-reveal">
            <!-- 🌐 LANG: Newsroom head -->
            <span class="ix-sw-tag"><?= t('news_tag', 'Live Newsroom') ?></span>
            <h2><?= t('news_title', 'Berita, Agenda & Pengumuman') ?></h2>
            <p><?= t('news_desc', 'Informasi terkini seputar kegiatan penjaminan mutu LPM.') ?></p>
        </div>
        ...
        <div class="ix-news-ticker">
            <div class="ix-news-ticker-track">
                ...
                for ($i = 0; $i < 2; $i++):
                    foreach ($tickers as $t_item): ?>
                    <div class="ix-news-ticker-item">
                        <b>●</b>
                        <?= Security::e(mb_strimwidth($t_item['judul'], 0, 70, '...')) ?>
                    </div>
                <?php endforeach; endfor; ?>
            </div>
        </div>
        ...
        <div class="ix-quote ix-reveal">
            <div class="ix-quote-grid">
                ...
                <div>
                    ...
                    <cite>
                        <strong><?= Security::e($kepalaLpm['nama_lengkap'] ?? t('quote_fallback', 'Kepala LPM')) ?></strong>
                        <small><?= t('quote_position', 'Kepala Lembaga Penjaminan Mutu') ?></small>
                    </cite>
                </div>
            </div>
        </div>
    ...
    <div class="container">
        <div class="ix-mega-inner ix-reveal">
            ...
            <span class="ix-eyebrow" style="background:rgba(201,162,39,.15);border-color:rgba(201,162,39,.3);color:var(--accent-light);">
                <span class="ix-eyebrow-dot"></span> <?= t('mega_eyebrow', 'Mari Berkolaborasi') ?>
            </span>
            <h2 style="margin-top:20px;">
                <?= t('mega_title_1', 'Wujudkan') ?> <span class="ix-grad"><?= t('mega_title_2', 'Budaya Mutu') ?></span><br>
                <?= t('mega_title_3', 'Bersama Kami') ?>
            </h2>
            <p><?= t('mega_desc', 'Masukan Anda adalah bahan bakar peningkatan mutu. Sampaikan aspirasi atau masuk ke portal SIM-Mutu bagi civitas akademika.') ?></p>
            <div class="ix-mega-ctas">
                <a href="/publik/pengaduan.php" class="ix-btn-glow ix-btn-primary"><?= t('mega_cta_1', '📨 Sampaikan Masukan') ?></a>
                <a href="/login.php" class="ix-btn-glow ix-btn-ghost"><?= t('mega_cta_2', '🔐 Masuk SIM-Mutu →') ?></a>
            </div>
        </div>
    </div>
</section>
